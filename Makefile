...
CFLAGS += -Wa,-adhlns=$(addprefix $(BLDDIR)/,$(<F:.c=.lst))
CFLAGS += $(patsubst %,-I%,$(EXTRAINCDIRS))
CFLAGS += $(patsubst %,-I%,$(QP_INCDIR))
CFLAGS += $(patsubst %,-I%,$(QP_PRTDIR))
CFLAGS += $(CSTANDARD)
...
LDFLAGS = -Wl,-Map=$(BLDDIR)/$(TARGET).map,--cref
LDFLAGS += $(EXTMEMOPTS)
...
AVRDUDE_WRITE_FLASH = -U flash:w:$(BLDDIR)/$(TARGET).hex
...
AVRDUDE_FLAGS += -D -b $(AVRDUDE_SPEED)
...
GENDEPFLAGS = -MD -MP -MF $(BLDDIR)/$(*F).d
...
HEXSIZE = $(SIZE) --target=$(FORMAT) $(BLDDIR)/$(TARGET).hex
ELFSIZE = $(SIZE) --format=avr $(BLDDIR)/$(TARGET).elf

sizebefore:
	@if test -f $(BLDDIR)/$(TARGET).elf; then echo; echo $(MSG_SIZE_BEFORE); $(ELFSIZE); \
	2>/dev/null; echo; fi

sizeafter:
	@if test -f $(BLDDIR)/$(TARGET).elf; then echo; echo $(MSG_SIZE_AFTER); $(ELFSIZE); \
	2>/dev/null; echo; fi
...
$(BLDDIR)/%.lss: $(BLDDIR)/%.elf
	@echo
	@echo $(MSG_EXTENDED_LISTING) $@
	$(OBJDUMP) -h -S $< > $@

# Create a symbol table from ELF output file.
$(BLDDIR)/%.sym: $(BLDDIR)/%.elf
	@echo
...
$(BLDDIR)/%.elf: $(OBJ) | dir
	@echo
	@echo $(MSG_LINKING) $@
	$(CC) $(ALL_CFLAGS) $(addprefix $(BLDDIR)/,$(^F)) --output $@ $(LDFLAGS)
